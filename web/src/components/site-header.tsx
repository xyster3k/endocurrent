...
export function SiteHeader() {
  const pathname = usePathname();
   // public menus for dropdown; safe fallback to empty.
  const [menuItems, setMenuItems] = useState<MenuItem[]>([]);
  useEffect(() => {
    fetch("/api/menus")
      .then((res) => res.json())
      .then((res) => {
        if (res?.data?.items) setMenuItems(res.data.items as MenuItem[]);
      })
      .catch(() => {});
  }, []);
      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
        ...
        <nav className="hidden items-center gap-6 text-sm font-medium md:flex">
          <div className="relative group">
            <button
              className="inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-slate-700 transition-colors hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800"
              aria-haspopup="true"
            >
            ...
            {menuItems.length > 0 ? (
              <div className="invisible absolute left-1/2 z-30 mt-2 w-72 -translate-x-1/2 rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl opacity-0 transition group-hover:visible group-hover:opacity-100 dark:border-slate-700 dark:bg-slate-900">
                ...
                <div className="space-y-2">
                  ...
                  {menuTree["root"]?.length === 0 && <p className="text-sm text-slate-500">No menu defined yet.</p>}
                </div>
              </div>
            ) : null}
          </div>
            ...
            <Link
              ...
              href={link.href}
                className={cn(
                  ...
                  pathname === link.href && "btn-primary shadow-sm"
                )}
            >
              {link.label}
            </Link>
